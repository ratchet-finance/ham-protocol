...
// Modified to work in the SPAM system

// all votes work on underlying _spamBalances[address], not balanceOf(address)
...
//    Since SPAM supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match SPAM as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "SPAM Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(spam.initSupply(), 4), 100); } // 4% of SPAM

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(spam.initSupply(), 100); } // 1% of SPAM
    ...
    constructor(address timelock_, address spam_) public {
        timelock = TimelockInterface(timelock_);
        spam = SPAMInterface(spam_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(spam.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || spam.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = spam.getPriorVotes(voter, proposal.startBlock);
